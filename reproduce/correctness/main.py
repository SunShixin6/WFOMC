...
# python reproduce/correctness/main.py
...
from reproduce.correctness.benchmarks import (
    DEFAULT_DELTA,
    DEFAULT_EPSILON,
    DEFAULT_FLUSH_EVERY,
...
from reproduce.correctness.runner import run_experiment
from reproduce.utils.smoke_profile import apply_smoke_correctness_config, is_smoke_mode
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if is_smoke_mode(args.smoke):
        config = apply_smoke_correctness_config(config)
        print("Smoke mode enabled for correctness.main")

    timestamp = datetime.now(ZoneInfo(TIMEZONE)).strftime("%Y%m%d_%H%M%S")
    results_path = config.raw_data_results_path / f"results_{timestamp}"

    results_path.mkdir(parents=True, exist_ok=True)
    config.raw_data_results_path.mkdir(parents=True, exist_ok=True)
    config.appendix_b1_results_path.mkdir(parents=True, exist_ok=True)
    config.cnf_results_root.mkdir(parents=True, exist_ok=True)

    print(f"The experimental results will be saved in:{results_path}")
    run_experiment(
        config=config,
