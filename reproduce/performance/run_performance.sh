...
print_usage() {
  cat <<'EOF'
Usage: run_performance.sh [args...]

Run reproduce.performance.main only.
...
RUN_LOG_FILE="${RUN_LOG_DIR}/performance_main.log"
RUN_META_FILE="${RUN_LOG_DIR}/performance_main.meta.json"

trap 'exit_code=$?; repro_write_meta "${RUN_META_FILE}" "${REPO_ROOT}" "reproduce/performance/run_performance.sh" "${RUN_START_TIME}" "${RUN_LOG_DIR}" "${RUN_LOG_FILE}" "${exit_code}"' EXIT
...
if ! repro_run_and_tee "reproduce.performance.main" "${RUN_LOG_FILE}" env PYTHONPATH=src:. "${PYTHON_CMD[@]}" -m reproduce.performance.main "$@"; then
  exit 1
fi
