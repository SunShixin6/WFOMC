...
def run_single(model_name, model_csv_name, domain_size, algo, r=None, k=None):
    ...
    if problem.cardinality_constraint and problem.cardinality_constraint.constraints:
        ## 这里默认基数约束和domain_size 是相同的。
        # 根据domain_size 构造新的constraints
        new_constraints = []
        for old_constraint in problem.cardinality_constraint.constraints:
            new_constraint = (old_constraint[0], old_constraint[1], domain_size)
            new_constraints.append(new_constraint)
        problem.cardinality_constraint.constraints = tuple(new_constraints)
...
class Config:
    ...
    GROUPS = [
        ...
        # }
        {
            ...
            "algorithms": ["dr", "fast"],
            "models": {
                "perm": "permutation-no-fix.wfomcs",
            }
        }
    ]
