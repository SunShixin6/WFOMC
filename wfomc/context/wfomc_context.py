...
from symengine import I, exp, symbols, pi  # 新导入的
...
def generate_D(domain_size, var_counts):  # var_counts 是一个列表，其中包含每个公式的变量数量
    ...
    """
    ranges = [range(int(domain_size ** var_count + 1)) for var_count in var_counts]  # ranges 中的每个 range 表示每个公式可以生成的计数范围。
    D = [tuple(x for x in combo) for combo in product(*ranges)]  # 将所有可能的组合转换为包含 Rational 类型元素的元组
    return D


def generate_M(Delta, vars_list):
    return [Delta ** vars_count + Rational(1, 1) for vars_count in vars_list]
...
class WFOMCContext(object):
    ...
    def __init__(self, problem: WFOMCProblem):
        self.domain: set[
            Const] = problem.domain  # 这是一个 set 集合，包含问题中的常量 Const。这些常量定义了域的范围。{'domain0', 'domain1', 'domain2', 'domain3', 'domain4', 'domain5', 'domain6', 'domain7', 'domain8', 'domain9'}
        self.sentence: SC2 = problem.sentence  # 这是 SC2 类型的句子，表示问题中涉及的逻辑公式。
        self.weights: dict[
            Pred, tuple[Rational, Rational]] = problem.weights  # self.weights 是一个字典，用于存储谓词（Pred）及其对应的权重对（tuple[Rational, Rational]）。这个权重对通常表示谓词为真的权重和为假的权重。这里将从 problem 对象中获取权重并赋值给 self.weights。
    ...
    def get_dft_weight(self, k_M, pred: Pred) -> tuple[RingElement, RingElement]:
        default = Rational(1, 1)
    ...
    def get_coef(self):
        ...
        for _, item in enumerate(pred_arity_dict):
            n.append(pred_cons_dict[item])
            var_counts.append(pred_arity_dict[item])
        D = generate_D(len(self.domain), var_counts)  # 获取D = {0, 1, . . . , |∆||vars(α1)|} × · · · × {0, 1, . . . , |∆||vars(αm)|}.
        M = generate_M(len(self.domain), var_counts)  # M = [|∆||vars(α1)| + 1, . . . , |∆||vars(αm)| + 1, 1],
        return n, D, M

    def get_dot(self, n: list, k: tuple, M: list):
        tmp = Rational(0, 1)  # 确保每次计算都使用 Rational 类型
        for i in range(len(n)):
            # 将 n[i]、k[i] 和 M[i] 转换为 Rational 类型
            n_val = Rational(n[i])  # 将 n[i] 转换为 Rational
            k_val = Rational(k[i])  # 将 k[i] 转换为 Rational
            M_val = Rational(M[i])  # 将 M[i] 转换为 Rational
            tmp += n_val * k_val / M_val  # 执行有理数类型的运算
        return tmp
