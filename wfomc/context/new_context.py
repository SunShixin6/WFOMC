...
from wfomc.utils.third_typing import Rational
...
class WFOMCContextNewEncoding(WFOMCContext):
    ...
    def __init__(self, problem: WFOMCProblem, **kwargs):
        super().__init__(problem, **kwargs) # 初始化方法，直接调用父类的初始化，行为与旧 Context 相同。
    ...
    def _build(self):
        ...
        ext_formulas_to_skolemize = list(self.sentence.ext_formulas) # 初始化待 Skolem 化的公式列表
        ...
        if self.sentence.contain_counting_quantifier():
            ...
            for cnt_formula in self.sentence.cnt_formulas: # 遍历语句中所有的计数量词公式
                ## 2.1 【判断类型】检查计数量词是一元还是二元
                is_unary_counting = not isinstance(cnt_formula.quantified_formula, QuantifiedFormula) #如果内部不是 QuantifiedFormula，则为一元

                ## 2.2 【情况 A】如果是一元计数 (例如 ∃=k X: P(X))
                if is_unary_counting:
                    logger.info(f"Handling unary counting formula (direct conversion): {cnt_formula}")
                ...
                ## 2.3 【情况 B】如果是二元计数 (例如 ∀X ∃=k Y: R(X,Y))
                    ...
                    logger.info(f"Handling binary counting formula with NEW encoding: {cnt_formula}")
                    
                    ## 2.3.1 调用旧的转换函数，获取旧编码 Γ* 的所有组件
                    uni_formula_old, ext_formulas_from_cnt, card_constraint, repeat_factor = convert_counting_formula(cnt_formula, self.domain) 
                    ...
                    k = cnt_formula.quantified_formula.quantifier_scope.count_param # 提取计数参数 k，并提前创建新编码所需的谓词
                    
                    ## 2.3.2 立即处理与该计数量词相关的 k 个存在量词公式
                    skolem_preds = [] # 用于存储 k 个 Aᵢ 谓词
                    skolem_axioms = top # 用于收集 k 个 Skolem 化公理
                    for ext_formula in ext_formulas_from_cnt: # 遍历 k 个形如 ∀X∃Y:fᵢ(X,Y) 的公式
                        inner_formula = ext_formula.quantified_formula.quantified_formula # 提取 fᵢ(X,Y) 部分
                        skolem_pred = new_predicate(1, SKOLEM_PRED_NAME) # 为当前的 fᵢ 创建一个唯一的 Skolem 谓词 (Aᵢ)
                        skolem_preds.append(skolem_pred) # 保存 Aᵢ 以便后续使用
                        X = Var('X')
                        axiom_body = skolem_pred(X) | ~inner_formula # 生成 Skolem 化公理: Aᵢ(X) ∨ ¬fᵢ(X,Y)
                        skolem_axioms &= axiom_body # 将公理加入集合
                        self.weights[skolem_pred] = (Rational(1, 1), Rational(-1, 1)) # 为新创建的 Aᵢ 设置 (1, -1) 的权重
                    ...
                    for j in range(k + 1):
                        ...
                        gamma_c_body &= c_preds[j](X).equivalent(definition) # 将 Cⱼ(X) ⇔ definition 这条规则加入到总约束中
                    
                    ## 2.3.5 【构建强制析取】构建 ∀x ⋁ Cⱼ(x) 规则，强制模型必须是规范的
                    disjuncts = [p(X) for p in c_preds] # 使用 reduce 和 | 操作符来构建析取。 这部分实现了论文中的公式: ∀x ⋁ Cⱼ(x)
                    final_disjunction_body = reduce(lambda f1, f2: f1 | f2, disjuncts, bot)

                    ## 2.3.6 【组合】将所有部分组合成最终的无量词公式
                    self.formula &= uni_formula_old & skolem_axioms & gamma_c_body & final_disjunction_body
        ...
        if self.contain_cardinality_constraint():
            self.weights.update(self.cardinality_constraint.transform_weighting(self.get_weight))
