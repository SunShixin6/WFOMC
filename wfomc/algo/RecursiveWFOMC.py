...
from sympy import factorint, \
    factor_list  # sympy 是一个符号数学库，提供了factorint和factor_list用于因式分解。factorint: 将整数进行因式分解，返回分解结果。例如，factorint(12) 会返回 {2: 2, 3: 1}，表示 12 = 2² × 3。factor_list: 对多项式进行因式分解。它返回一个分解后的多项式列表。
...
def print_tree(node: TreeNode):
    with open("./log.txt", "a") as f:
        if node.value:
            print("  " * node.depth, "depth: ", node.depth, " ", "value: ", node.value, file=f)  # 打印当前节点的深度和它的权重（cell_weights）。深度越大，缩进越多。
...
def dfs_wfomc_real(cell_weights, domain_size,
    ...
    for l in range(CELLS_NUM):  # 代码遍历一个范围为 CELLS_NUM 的循环，表示程序正处理多个“细胞”。
        ...
        new_cell_weights = my_simplify([expand(cell_weights[i] * ORI_WEIGHT_ADJ_MAT[l][i]) for i in
                                        range(CELLS_NUM)])  # 计算新的细胞权重 new_cell_weights。具体方法是遍历 CELLS_NUM 内的每个 i，将 cell_weights[i] 与 ORI_WEIGHT_ADJ_MAT[l][i] 相乘后 # 调用 expand 函数来调整和扩展权重。这生成了一个新的细胞权重列表。
        ...
        if domain_size - 1 == 1:
            ...
            if ENABLE_ISOMORPHISM:  # 如果启用了 ENABLE_ISOMORPHISM（同构处理），代码会调整顶点颜色，使颜色编号从 0 开始并连续。
                ...
                if tuple(
                        adjust_vertex_colors) not in CACHE_FOR_NAUTY:  # 判断 adjust_vertex_colors 的元组形式是否已经存在于 CACHE_FOR_NAUTY 中。# CACHE_FOR_NAUTY 是一个缓存，用来保存之前已经生成的图的特征，以避免重复计算 # 这个判断是为了确保对于一个已经处理过的颜色分配，我们不需要重复调用 pynauty 进行图同构检查。
                    can_label = pynauty.certificate(update_graph(extend_vertex_coloring(adjust_vertex_colors,
                                                                                        no_color)))  # 使用certificate()函数生成图的同构证书，传入的参数是调用 update_graph() 和 extend_vertex_coloring() 函数处理过的顶点颜色数据。# 首先，调用 extend_vertex_coloring(adjust_vertex_colors, no_color) 扩展顶点颜色，以便图转换过程中可以准确反映每个顶点的颜色信息。# extend_vertex_coloring 会根据层数将顶点颜色扩展为多层结构，使每层的顶点颜色信息可以反映在多层图中。# update_graph 将更新后的顶点颜色信息设置到图中（GRAPH），使其可以用于图同构检查。
