...
def incremental_wfomc(context: WFOMCContext,
    ...
    for cell_graph, weight in build_cell_graphs(formula, get_weight, leq_pred=leq_pred):  # 从这里开始相当于对算法进行一次或者多次求和（若有多个cell_graph），目前只有一个cellgraph，相当于没有遍历
        cells = cell_graph.get_cells()
        n_cells = len(cells)
        T = defaultdict(lambda: Rational(0, 1))  ## 新建一个空字典，用于存储 dp过程的状态
        ...
        for i in range(2, domain_size + 1):  # domain中剩下的元素
            ...
            for j, cell in enumerate(cells):  # 遍历每个可以放元素的cell type C_j。
                ...
                for (d_old, k_old), w_old in old_T.items():
                    # if w_old == 0.0:  # w_old = 0 可以跳过， # 这里在存储数据的时候，才注释掉，让w=0的也存储。正式运行计算WFOMC的，需要使用
                    #     continue
                    ...
                    for d_delta, w_delta in get_coef(symbolic_weight, gen_vars):
                        ...
                        w_new = w_old * w_delta ## 这里w_deta 是上一层相对于下一层的系数，因为只增加了一个点，
        ...
        # section：对dp最终得到的结果求和，选择符合要求的，为WFOMC结果
