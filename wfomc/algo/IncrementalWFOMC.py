...
def incremental_wfomc(context: WFOMCContext,
    ...
    for cell_graph, weight in build_cell_graphs(formula, get_weight, leq_pred=leq_pred):  # 通过 build_cell_graphs 函数生成一组 cell_graph 和对应的 weight，该函数的参数包括 formula、get_weight 和可选的 leq_pred。
        ...
        # # 初始化DP表
        # dp_table = defaultdict(lambda: Rational(0, 1))  # 使用defaultdict初始化DP表dp_table，它将状态(ivec, ccs)映射到相应的RingElement值。
        # # 初始化起始状态 (ivec 和 ccs 都为零)
        # dp_table[(tuple([0] * domain_size), tuple([0] * len(domain)))] = Rational(1, 1)
        #
        # # 生成所有可能的ivec配置, # 遍历所有可能的配置 ivec，通过 multinomial 函数生成配置。
        ...
        # 下面是原始代码
        table = dict((tuple(int(k == i) for k in range(n_cells)), cell_graph.get_cell_weight(cell)) for i, cell in enumerate(cells))  # 初始化 table 字典，其中键是元组，表示单元格在图中的状态。每个元组的元素是 int(k == i)，用于表示每个单元格的状态（例如是否为目标单元格）。字典的值是该单元格的权重，使用 cell_graph.get_cell_weight(cell) 获取。
